...
namespace Gurux.DLMS.AMI.Client
{
    ...
    public class GXNotifierService : ComponentBase, IGXNotifier2
    {
        ...
        public GXNotifierService(IServiceProvider services)
        {
            ...
            _toasterService = services.GetService<IGXToasterService>();
            dataList = new Dictionary<string, object?>();
            Items = new List<GXMenuItem>();
            NavigationManager? navigationManager = services.GetService<NavigationManager>();
            if (navigationManager != null)
            {
                ...
                {
                    //NavigationManager is not available on the server side.
        ...
        public EditContext? EditContext
        {
            ...
            set
            {
                if (_editContext != null && OnDirty != null)
                {
                    _editContext.OnFieldChanged -= (a, b) =>
                    {
                    };
                }
                _editContext = value;
            ...
            lock (_stamps)
            {
                foreach (var it in _stamps)
                {
                    if (it.TargetType == type)
                    {
                        ...
                        else if (datetime > it.CreationTime)
                        {
                            it.CreationTime = datetime;
                            it.Errors += errors;
        ...
        ///<inheritdoc />
        public async Task Mark(string type,
            DateTimeOffset? datetime)
        {
            bool reset = false;
            try
            {
                if (datetime != null)
                {
                    ...
                    if (!reset)
                    {
                        var userStampRepository = _services.GetRequiredService<IUserStampRepository>();
                        await userStampRepository.UpdateAsync([stamp]);
                    }
                }
            }
            catch (Exception ex)
        ...
        ///<inheritdoc />
        public async Task Mark(HttpClient http, string type, DateTimeOffset? datetime)
        {
            bool reset = false;
            try
            {
        ...
        public IEnumerable<GXUserStamp> GetUserStamps(IEnumerable<string>? types)
        {
            if (types?.Any() == true)
            {
                lock (_stamps)
                {
                    return _stamps.Where(w => types.Contains(w.TargetType));
                }
            }
            return [];
        }
        ...
        {
            return Notifications == null || !Notifications.Contains(type);
        }

        public async Task StartAsync(ClaimsPrincipal User,
            IServiceScope scope,
            HttpClient http,
            ...
            try
            {
                ...
                */
                if (hubConnection == null)
                {
                   ...
                   .WithUrl(navigationManager.ToAbsoluteUri("/guruxami"))
                   ...
                   .Build();
                    ...
                    request.Filter = new GXNotification()
                    {
                        Delivery = (UInt16)NotificationDelivery.UI
                    };
                    GXNotification[]? notifications = null;
                    var nr = scope.ServiceProvider.GetService<INotificationRepository>();
                    if (nr != null)
                    {
                        notifications = await nr.ListAsync(request);
                    }
                    else
                    {
                        try
                        {
                            var ret2 = await http.PostAsJson<ListNotificationResponse>("api/Notification/List", request);
                            notifications = ret2.Notifications;
                        }
                        catch (Exception)
                        {
                            //Pass
                    ...
                    {
                        string?[]? list = notifications.Select(s => s.Targets).ToArray();
                        bool all = list == null || !list.Any();
                        ...
                        if (list != null)
                        {
                            foreach (var it in list)
                            {
                                ...
                                {
                                    arr.AddDistinct2(it.Split([';', ','], StringSplitOptions.RemoveEmptyEntries));
                                }
                            }
                            Notifications = arr.ToArray();
                        }
                    }
                    _logger.LogInformation("Notifications: " + Notifications);
                    GXUser? user;
                    var ur = scope.ServiceProvider.GetService<IUserRepository>();
                    if (ur != null)
                    {
                        user = await ur.ReadAsync(null);
                    }
                    else
                    {
                        try
                    ...
                    {
                        var settings = user.Settings.Where(w => w.Name == GXConfigurations.Performance).SingleOrDefault();
                        settings = user.Settings.Where(w => w.Name == GXConfigurations.System).SingleOrDefault();
                    ...
                    hubConnection.On(nameof(IGXHubEvents.ClearSystemLogs), async () =>
                    {
                        ...
                        await UpdateUserStamp(TargetType.SystemLog, null, 0, 0, 0, 0);
                    });
                    ...
                    {
                        if (!IsIgnored(TargetType.DeviceLog))
                        {
                        ...
                        await UpdateUserStamp(TargetType.DeviceLog, null, 0, 0, 0, 0);
                    });
                    hubConnection.On<IEnumerable<GXDeviceError>>(nameof(IGXHubEvents.AddDeviceErrors), async (errors) =>
                    {
                        ...
                        await UpdateUserStamp(TargetType.DeviceLog,
                    ...
                    hubConnection.On<IEnumerable<GXWorkflow>?>(nameof(IGXHubEvents.ClearWorkflowLogs), async (workflows) =>
                    {
                        ...
                        await UpdateUserStamp(TargetType.WorkflowLog, null, 0, 0, 0, 0);
                    });
                    hubConnection.On<IEnumerable<GXWorkflowLog>>(nameof(IGXHubEvents.AddWorkflowLogs), async (errors) =>
                    {
                        ...
                        await UpdateUserStamp(TargetType.WorkflowLog,
                            errors.Max(s => s.CreationTime),
                            errors.Where(w => w.Level == (int)TraceLevel.Error).Count(),
                            errors.Where(w => w.Level == (int)TraceLevel.Warning).Count(),
                            errors.Where(w => w.Level == (int)TraceLevel.Info).Count(),
                    ...
                    hubConnection.On<IEnumerable<GXGateway>?>(nameof(IGXHubEvents.ClearGatewayLogs), async (gateways) =>
                    {
                        ...
                        await UpdateUserStamp(TargetType.GatewayLog, null, 0, 0, 0, 0);
                    });
                    hubConnection.On<IEnumerable<GXGatewayLog>>(nameof(IGXHubEvents.AddGatewayLogs), async (errors) =>
                    {
                        ...
                        await UpdateUserStamp(TargetType.GatewayLog,
                            errors.Max(s => s.CreationTime),
                            errors.Where(w => w.Level == (int)TraceLevel.Error).Count(),
                            errors.Where(w => w.Level == (int)TraceLevel.Warning).Count(),
                            errors.Where(w => w.Level == (int)TraceLevel.Info).Count(),
                    ...
                    hubConnection.On<IEnumerable<GXScript>>(nameof(IGXHubEvents.ClearScriptLogs), async (scripts) =>
                        {
                            ...
                            await UpdateUserStamp(TargetType.ScriptLog, null, 0, 0, 0, 0);
                        });
                    ...
                    hubConnection.On<IEnumerable<GXUser>>(nameof(IGXHubEvents.ClearUserErrors), async (users) =>
                    {
                        ...
                        await UpdateUserStamp(TargetType.UserError, null, 0, 0, 0, 0);
                    });
                    ...
                    hubConnection.On<IEnumerable<GXModule>>(nameof(IGXHubEvents.ClearModuleLogs), async (modules) =>
                    {
                        ...
                        await UpdateUserStamp(TargetType.ModuleLog, null, 0, 0, 0, 0);
                    });
                    ...
                    hubConnection.On<IEnumerable<GXAgent>?>(nameof(IGXHubEvents.ClearAgentLogs), async (logs) =>
                    {
                        ...
                        await UpdateUserStamp(TargetType.AgentLog, null, 0, 0, 0, 0);
                    });
                    ...
                    hubConnection.On<IEnumerable<GXSchedule>?>(nameof(IGXHubEvents.ClearScheduleLog), async (logs) =>
                    {
                        if (!IsIgnored(TargetType.ScheduleLog))
                        {
                            if (logs == null || !logs.Any())
                            ...
                            {
                                _toasterService?.Add(new GXToast("Schedule log clear", ToString(logs), Color.Info, 15));
                            }
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ClearScheduleLog), logs);
                        await UpdateUserStamp(TargetType.ScheduleLog, null, 0, 0, 0, 0);
                    });
                    ...
                    hubConnection.On<IEnumerable<GXContent>>(nameof(IGXHubEvents.ContentUpdate), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Content))
                        {
                            _toasterService?.Add(new GXToast("Content updated", ToString(items), Color.Info, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ContentUpdate), items);
                    });
                    hubConnection.On<IEnumerable<GXContent>>(nameof(IGXHubEvents.ContentDelete), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Content))
                        {
                            _toasterService?.Add(new GXToast("Content deleted", ToString(items), Color.Warning, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ContentDelete), items);
                    });
                    hubConnection.On<IEnumerable<GXContent>>(nameof(IGXHubEvents.ContentClose), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Content))
                        {
                            _toasterService?.Add(new GXToast("Content closed", ToString(items), Color.Info, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ContentClose), items);
                    });
                    hubConnection.On<IEnumerable<GXContentGroup>>(nameof(IGXHubEvents.ContentGroupUpdate), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Content))
                        {
                            _toasterService?.Add(new GXToast("Content group updated", ToString(items), Color.Info, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ContentGroupUpdate), items);
                    });
                    hubConnection.On<IEnumerable<GXContentGroup>>(nameof(IGXHubEvents.ContentGroupDelete), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Content))
                        {
                            _toasterService?.Add(new GXToast("Content group deleted", ToString(items), Color.Warning, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ContentGroupDelete), items);
                    });
                    ...
                    hubConnection.On<IEnumerable<GXPerformance>>(nameof(IGXHubEvents.PerformanceAdd), async (performances) =>
                    {
                        ...
                        await UpdateUserStamp(TargetType.Performance,
                            performances.Max(s => s.Start),
                            0,
                            0,
                            performances.Count(),
                            0);
                    ...
                    hubConnection.On(nameof(IGXHubEvents.PerformanceClear), async () =>
                    {
                        ...
                        await UpdateUserStamp(TargetType.Performance, null, 0, 0, 0, 0);
                        await ChangedAsync(nameof(IGXHubEvents.PerformanceClear));
                    });
                    hubConnection.On<IEnumerable<GXReport>>(nameof(IGXHubEvents.ReportUpdate), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Report))
                        {
                            _toasterService?.Add(new GXToast("Report updated", ToString(items), Color.Info, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ReportUpdate), items);
                    });
                    hubConnection.On<IEnumerable<GXReport>>(nameof(IGXHubEvents.ReportDelete), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Report))
                        {
                            _toasterService?.Add(new GXToast("Report deleted", ToString(items), Color.Warning, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ReportDelete), items);
                    });
                    hubConnection.On<IEnumerable<GXReport>>(nameof(IGXHubEvents.ReportCalculate), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Report))
                        {
                            _toasterService?.Add(new GXToast("Report calculated", ToString(items), Color.Info, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ReportCalculate), items);
                    });
                    hubConnection.On<IEnumerable<GXReportGroup>>(nameof(IGXHubEvents.ReportGroupUpdate), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Report))
                        {
                            _toasterService?.Add(new GXToast("Report group updated", ToString(items), Color.Info, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ReportGroupUpdate), items);
                    });
                    hubConnection.On<IEnumerable<GXReportGroup>>(nameof(IGXHubEvents.ReportGroupDelete), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Report))
                        {
                            _toasterService?.Add(new GXToast("Report group deleted", ToString(items), Color.Warning, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ReportGroupDelete), items);
                    });
                    hubConnection.On<IEnumerable<GXReport>?>(nameof(IGXHubEvents.ClearReportLogs), async (logs) =>
                    {
                        if (!IsIgnored(TargetType.Report))
                        {
                            if (!IsIgnored(TargetType.Report))
                            {
                                if (logs == null || !logs.Any())
                                {
                                    _toasterService?.Add(new GXToast("Report logs cleared", "All logs cleared.", Color.Info, 15));
                                }
                                else
                                {
                                    _toasterService?.Add(new GXToast("Report log clered", ToString(logs), Color.Info, 15));
                                }
                            }
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ClearReportLogs), logs);
                        await UpdateUserStamp(TargetType.ReportLog, null, 0, 0, 0, 0);
                    });
                    hubConnection.On<IEnumerable<GXReportLog>>(nameof(IGXHubEvents.AddReportLogs), async (logs) =>
                    {
                        if (!IsIgnored(TargetType.ReportLog))
                        {
                            _toasterService?.Add(new GXToast("Report log added", ToString(logs), Color.Warning, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.AddReportLogs), logs);
                        await UpdateUserStamp(TargetType.ReportLog,
                            logs.Max(s => s.CreationTime),
                            logs.Where(w => w.Level == (int)TraceLevel.Error).Count(),
                            logs.Where(w => w.Level == (int)TraceLevel.Warning).Count(),
                            logs.Where(w => w.Level == (int)TraceLevel.Info).Count(),
                    ...
                    hubConnection.On<IEnumerable<GXReportLog>>(nameof(IGXHubEvents.CloseReportLogs), async (items) =>
                    {
                        if (!IsIgnored(TargetType.ReportLog))
                        {
                            _toasterService?.Add(new GXToast("Report log closed", ToString(items), Color.Info, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.CloseReportLogs), items);
                    });
                    ...
                    hubConnection.On<IEnumerable<GXNotification>>(nameof(IGXHubEvents.NotificationUpdate), async (notifications) =>
                    {
                        foreach (var it in notifications)
                        {
                            if (it.Delivery == (UInt16)NotificationDelivery.UI)
                            {
                                //If user has updated UI settings.
                                if (string.IsNullOrEmpty(it.Targets))
                                {
                                    Notifications = null;
                                }
                                else
                                {
                                    Notifications = it.Targets.Split([';', ','], StringSplitOptions.RemoveEmptyEntries);
                                }
                                break;
                            }
                        }

                        if (!IsIgnored(TargetType.Notification))
                        {
                            _toasterService?.Add(new GXToast("Notifications updated", ToString(notifications), Color.Info, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.NotificationUpdate), notifications);
                    });
                    hubConnection.On<IEnumerable<GXNotification>>(nameof(IGXHubEvents.NotificationDelete), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Notification))
                        {
                            _toasterService?.Add(new GXToast("Notification deleted", ToString(items), Color.Warning, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.NotificationDelete), items);
                    });
                    hubConnection.On<IEnumerable<GXNotificationGroup>>(nameof(IGXHubEvents.NotificationGroupUpdate), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Notification))
                        {
                            _toasterService?.Add(new GXToast("Notification group updated", ToString(items), Color.Info, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.NotificationGroupUpdate), items);
                    });
                    hubConnection.On<IEnumerable<GXNotificationGroup>>(nameof(IGXHubEvents.NotificationGroupDelete), async (items) =>
                    {
                        if (!IsIgnored(TargetType.Notification))
                        {
                            _toasterService?.Add(new GXToast("Notification group deleted", ToString(items), Color.Warning, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.NotificationGroupDelete), items);
                    });
                    hubConnection.On<IEnumerable<GXNotification>?>(nameof(IGXHubEvents.ClearNotificationLogs), async (logs) =>
                    {
                        if (!IsIgnored(TargetType.Notification))
                        {
                            if (!IsIgnored(TargetType.Notification))
                            {
                                if (logs == null || !logs.Any())
                                {
                                    _toasterService?.Add(new GXToast("Notification logs cleared", "All logs cleared.", Color.Info, 15));
                                }
                                else
                                {
                                    _toasterService?.Add(new GXToast("Notification log clered", ToString(logs), Color.Info, 15));
                                }
                            }
                        }
                        await ChangedAsync(nameof(IGXHubEvents.ClearNotificationLogs), logs);
                        await UpdateUserStamp(TargetType.NotificationLog, null, 0, 0, 0, 0);
                    });
                    hubConnection.On<IEnumerable<GXNotificationLog>>(nameof(IGXHubEvents.AddNotificationLogs), async (logs) =>
                    {
                        if (!IsIgnored(TargetType.NotificationLog))
                        {
                            _toasterService?.Add(new GXToast("Notification log added", ToString(logs), Color.Warning, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.AddNotificationLogs), logs);
                        await UpdateUserStamp(TargetType.NotificationLog,
                            logs.Max(s => s.CreationTime),
                            logs.Where(w => w.Level == (int)TraceLevel.Error).Count(),
                            logs.Where(w => w.Level == (int)TraceLevel.Warning).Count(),
                            logs.Where(w => w.Level == (int)TraceLevel.Info).Count(),
                    ...
                    hubConnection.On<IEnumerable<GXAppearance>>(nameof(IGXHubEvents.AppearanceUpdate), async (appearances) =>
                    {
                        if (!IsIgnored(TargetType.Appearance))
                        {
                            _toasterService?.Add(new GXToast("Appearance updated", ToString(appearances), Color.Warning, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.AppearanceUpdate), appearances);
                    });

                    hubConnection.On<IEnumerable<GXAppearance>>(nameof(IGXHubEvents.AppearanceDelete), async (appearances) =>
                    {
                        if (!IsIgnored(TargetType.Appearance))
                        {
                            _toasterService?.Add(new GXToast("Appearances delete", ToString(appearances), Color.Warning, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.AppearanceDelete), appearances);
                    });

                    hubConnection.On<IEnumerable<GXLocalizedResource>>(nameof(IGXHubEvents.LocalizedResourceUpdate), async (resources) =>
                    {
                        if (!IsIgnored(TargetType.LocalizedResource))
                        {
                            _toasterService?.Add(new GXToast("Localized resource updated", ToString(resources), Color.Warning, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.LocalizedResourceUpdate), resources);
                    });

                    hubConnection.On<IEnumerable<GXLocalizedResource>>(nameof(IGXHubEvents.LocalizedResourceDelete), async (resources) =>
                    {
                        if (!IsIgnored(TargetType.LocalizedResource))
                        {
                            _toasterService?.Add(new GXToast("Localized resource delete", ToString(resources), Color.Warning, 15));
                        }
                        await ChangedAsync(nameof(IGXHubEvents.LocalizedResourceUpdate), resources);
                    });
                    ...
                    GXUserStamp[]? list2 = null;
                    var us = scope.ServiceProvider.GetService<IUserStampRepository>();
                    if (us != null)
                    {
                        list2 = await us.ListAsync(req2);
                    }
                    else
                    {
                        try
                        {
                            var ret3 = await http.PostAsJson<ListUserStampsResponse>("api/UserStamp/List", req2);
                            list2 = ret3.Stamps;
                        }
                        catch (Exception)
                        {
                    ...
                    if (list2 != null)
                    {
                        lock (_stamps)
                        {
                            var idsInList2 = _stamps.Select(x => x.Id).ToHashSet();
                            list2 = list2.Where(x => !idsInList2.Contains(x.Id)).ToArray();
                            _stamps.AddRange(list2);
                            list2 = _stamps.ToArray();
                        }
                        await ChangedAsync(nameof(IGXHubEvents.StampUpdate), list2);
                    }
