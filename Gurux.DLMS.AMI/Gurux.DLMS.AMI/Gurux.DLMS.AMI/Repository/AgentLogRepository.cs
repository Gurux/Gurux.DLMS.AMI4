...
namespace Gurux.DLMS.AMI.Server.Repository
{
    ...
    class AgentLogRepository : IAgentLogRepository
    {
            ...
            ISystemLogRepository systemLogRepository)
        {
            ...
            if (user == null ||
                (!user.HasScope(GXAgentLogPolicies.Add) &&
                !user.IsInRole(GXRoles.Admin) &&
                !user.IsInRole(GXRoles.AgentLog) &&
                !user.IsInRole(GXRoles.AgentLogManager)))
            {
        ...
        /// <inheritdoc />
        public async Task AddAsync(string type, IEnumerable<GXAgentLog> errors)
        {
            ...
            {
                it.CreationTime = now;
                it.Type = await _enumTypeRepository.GetLogTypeAsync(TargetType.AgentLog, type);
                updates[it] = await _agentRepository.GetUsersAsync(it.Agent.Id);
            }
            ...
            foreach (var it in updates)
            {
                ...
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.AgentLog, NotificationAction.Add);
                if (users != null && users.Any())
                {
                    await _eventsNotifier.AddAgentLogs(users, [tmp]);
                }
            }
        }

        /// <inheritdoc />
        public async Task<GXAgentLog> AddAsync(string type, GXAgent agent, Exception ex)
        {
            DateTime now = DateTime.Now;
            Dictionary<GXAgentLog, List<string>> updates = new Dictionary<GXAgentLog, List<string>>();
            GXAgentLog error = new GXAgentLog(TraceLevel.Error)
            {
                ...
                Message = ex.Message,
                Agent = agent,
                Type = await _enumTypeRepository.GetLogTypeAsync(TargetType.AgentLog, type)
            };
            await _host.Connection.InsertAsync(GXInsertArgs.Insert(error));
            updates[error] = await _agentRepository.GetUsersAsync(agent.Id);
            foreach (var it in updates)
            {
                await _eventsNotifier.AddAgentLogs(it.Value, [it.Key]);
            }
            return error;
        }
        ...
        public async Task ClearAsync(Guid[]? agents)
        {
            ...
            if (!admin)
            {
                ...
                arg = GXQuery.GetAgentsByUser(s => "*", id);
            }
            else
            {
            ...
            List<string>? users = await _agentRepository.GetUsersAsync(agents);
            if (admin && (agents == null || !agents.Any()))
            {
                //Admin clears all agent logs.
                _host.Connection.Truncate<GXAgentLog>();
                //Clear enum types.
                await _enumTypeRepository.DeleteAsync(TargetType.AgentLog);
                logs = null;
            }
            else if (logs.Any())
            {
            ...
            users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                users, TargetType.AgentLog, NotificationAction.Clear);
            if (users != null && users.Any())
            {
                await _eventsNotifier.ClearAgentLogs(users, logs);
            }
            await _systemLogRepository.AddAsync(TargetType.AgentLog, 
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetAgentErrorsByUser(s => "*", userId, null);
            }
            if (request != null)
            {
        ...
        /// <inheritdoc />
        public async Task<GXAgentLog> ReadAsync(Guid id)
        {
