    ...
    public class FavoriteRepository : IFavoriteRepository
    {
        ...
        public async Task DeleteAsync(IEnumerable<Guid> userGrouprs)
        {
            ...
            var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                [userId], TargetType.Favorite, NotificationAction.Remove);
            if (users != null && users.Any())
            {
                await _eventsNotifier.FavoriteDelete(users, list);
            }
        }
        ...
        /// <inheritdoc />
        public async Task<GXFavorite> ReadAsync(Guid id)
        {
            string userId = ServerHelpers.GetUserId(User);
            GXSelectArgs arg = GXSelectArgs.SelectAll<GXFavorite>();
            arg.Joins.AddInnerJoin<GXFavorite, GXUser>(j => j.User, j => j.Id);
            ...
            Expression<Func<GXFavorite, object?>>? columns)
        {
            ...
            foreach (GXFavorite it in favorites)
            {
                ...
                {
                    it.Type = it.Path;
                    if (string.Compare(it.Type, "Config", true) == 0)
                    {
                        ...
                        if (string.Compare(it.Type, "Module") != 0)
                        {
                            ...
                            switch (it.Type.ToLower())
                            {
                                case "modulegroup":
                                    ...
                                    break;
                                case "blockgroup":
                                    args = GXSelectArgs.Select<GXContentGroup>(s => s.Name, w => w.Id == Id);
                                    break;
                                case "component":
                ...
                if (id != null && target != null)
                {
                    ...
                    switch (target)
                    {
                        ...
                        case TargetType.Content:
                            args = GXSelectArgs.Select<GXContent>(s => s.Name, w => w.Id == Id);
                            break;
                        ...
                        case TargetType.Appearance:
                            args = GXSelectArgs.Select<GXAppearance>(s => new { s.Id }, w => w.Id == id);
                            break;
                    }
                }
                if (args != null)
                ...
                if (it.Id == Guid.Empty)
                {
                    ...
                    list.Add(it.Id);
                    //Remove user so it's not sent as part of the notification.
                    it.User = null;
                }
                else
                {
