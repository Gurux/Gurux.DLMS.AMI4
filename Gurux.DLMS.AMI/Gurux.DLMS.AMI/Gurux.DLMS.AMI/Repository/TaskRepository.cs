...
namespace Gurux.DLMS.AMI.Server.Repository
{
    ...
    class TaskRepository : ITaskRepository
    {
        ...
        /// <summary>
        /// Get task creator and admin users to notification.
        /// </summary>
        /// <param name="task"></param>
        /// <returns></returns>
        private async Task<List<string>> GetNofifiedUsersAsync(GXTask task)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.TaskManager) &&
                //If agent
            ...
            if (columns.TriggerUser != null)
            {
                ...
                {
                    users.AddDistinct(await _userRepository.GetUserIdsInRoleAsync([GXRoles.Admin]));
                }
            }
        ...
        public async Task DeleteAsync(IEnumerable<Guid> tasks)
        {
            ...
            {
                updates.Add(it, await GetNofifiedUsersAsync(it));
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.Task, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXTask tmp = new GXTask() { Id = it.Key.Id };
                await _eventsNotifier.TaskDelete(users, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (request != null)
            {
                if (request.Select != null && request.Select.Contains("Device"))
                {
                    ...
                    arg.Columns.Exclude<GXDevice>(e => e.Tasks);
                    arg.Columns.Exclude<GXAttributeTemplate>(e => new { e.ObjectTemplate, e.Updated, e.CreationTime, e.ExpirationTime, e.Name });
                    arg.Columns.Exclude<GXObjectTemplate>(e => new { e.DeviceTemplate, e.Updated, e.CreationTime, e.ExpirationTime, e.Name });
                    arg.Columns.Exclude<GXObject>(e => new { e.Device, e.Tasks, e.Attributes, e.CreationTime, e.LastError, e.LastErrorMessage, e.LastRead, e.LastAction, e.Updated });
        ...
        /// <inheritdoc />
        public async Task<GXTask> ReadAsync(Guid id)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.TaskManager) &&
        ...
        /// <inheritdoc />
        public async Task<Guid[]> AddAsync(IEnumerable<GXTask> tasks)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.TaskManager)))
            ...
            try
            {
                ...
                foreach (GXTask it in tasks)
                {
                    ...
                    {
                        GXSelectArgs args = GXQuery.GetDevicesByUser(s => s.Id, creator.Id, it.TargetDevice.Value);
                        ...
                        if (it.Object == null && it.Attribute == null)
                        {
                            ...
                            var list2 = await _objectRepository.ListAsync( req, null, default);
                ...
                foreach (GXTask it in tasks3)
                {
                    ...
                    if (it.TriggerSchedule == null && it.TriggerScript == null)
                    {
                    ...
                    {
                        await _scriptRepository.RunAsync(it.ScriptMethod.Id);
                    }
                    else if (it.Module != null)
                    {
                        ...
                        tmp = await _scheduleRepository.GetModuleSettingsAsync(tmp);
                        await _moduleService.ExecuteAsync(it.Module, tmp?.Settings);
                    }
                }
            ...
            await NotificationUsers(tasks3, true);
            return list.ToArray();
        }
        ...
        private async Task UpdateObjectExecutionTimes(GXTask executedTask, GXTask dbTask, GXObject obj)
        {
            ...
            }
            {
                ...
                await _objectRepository.UpdateAsync(new GXObject[] { obj }, columns);
            }
            {
                ...
                await _deviceRepository.UpdateAsync([obj.Device], CancellationToken.None, columns);
            }
        }

        /// <inheritdoc />
        public async Task DoneAsync(IEnumerable<GXTask> tasks)
        {
            foreach (GXTask it in tasks)
            {
                try
            ...
            foreach (GXTask it in tasks)
            {
                GXTask task = await ReadAsync(it.Id);
                if (task.Attribute != null)
                {
                    ...
                    {
                        case TaskType.Read:
                            //Read time is not update to profile generic buffer.
                            if (task.Attribute?.Object?.Template?.ObjectType == 7 &&
                                task.Attribute?.Template?.Index == 2)
                            {
                                columns = q => new { q.LastError, q.Exception };
                            }
                            else
                            {
                    ...
                    await _attributeRepository.UpdateAsync([task.Attribute], columns);
                    ...
                    await UpdateObjectExecutionTimes(it, task, obj);
                }
                if (task.Object != null)
                {
                    if (task.Index != 0)
                    {
                        ...
                        if (att != null)
                        {
                            ...
                            await _attributeRepository.UpdateAsync(new GXAttribute[] { att }, columns);
                        }
                    }
                    await UpdateObjectExecutionTimes(it, task, task.Object);
                }
            }
            await NotificationUsers(tasks);
        }

        /// <inheritdoc />
        public async Task RestartAsync(IEnumerable<GXTask> tasks)
        {
            foreach (GXTask it in tasks)
            {
        ...
        /// <summary>
        /// Notification users who can access the device that status of the task has changed.
        /// </summary>
        /// <param name="tasks">Notified tasks.</param>
        /// <param name="add">Is task added.</param>
        private async Task NotificationUsers(IEnumerable<GXTask> tasks,
            bool add = false)
        {
            List<string> users = new List<string>();
            List<GXTask> list = new List<GXTask>();
            ...
            {
                users.AddDistinct(await GetNofifiedUsersAsync(it));
            ...
            bool listener)
        {
            ...
            using (IServiceScope scope = _serviceProvider.CreateScope())
            {
                ...
                await repository.UpdateStatusAsync(agent.Id, connectionInfo, AgentStatus.Detected, null);
            }
            arg = SelectTasks();
            GXTask? task = null;
            if (DeviceId != null && DeviceId != Guid.Empty)
            {
                ...
                {
                    IDeviceRepository repository = scope.ServiceProvider.GetRequiredService<IDeviceRepository>();
                    await repository.UpdateStatusAsync(id, DeviceStatus.Connected);
                }
            }
            else if (gatewayId != null && gatewayId != Guid.Empty)
            {
                ...
                using (IServiceScope scope = _serviceProvider.CreateScope())
                {
                    ...
                    await repository.UpdateStatusAsync(id, GatewayStatus.Detected);
                }
            ...
            if (task != null)
            {
                ...
                {
                    IDeviceRepository repository = scope.ServiceProvider.GetRequiredService<IDeviceRepository>();
                    var users = (await repository.GetUsersAsync(task.TargetDevice.Value));
                }
                await NotificationUsers(notified);
            }
            return tasks;
        }
        ...
        public async Task ClearAsync()
        {
            ...
            //Clear enum types.
            await _enumTypeRepository.DeleteAsync(TargetType.Task);
            List<GXUser> clearedUsers = (await _host.Connection.SelectAsync<GXUser>(arg));
            var list = await _userRepository.GetUserIdsInRoleAsync([GXRoles.Admin]);
            //Notification users if their tasks are cleared.
            ...
            await _systemLogRepository.AddAsync(TargetType.Task,
