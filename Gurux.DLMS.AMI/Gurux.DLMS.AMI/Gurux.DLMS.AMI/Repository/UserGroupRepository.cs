    ...
    public class UserGroupRepository : IUserGroupRepository
    {
        ...
        public async Task<List<string>> GetUsersAsync(Guid? groupId)
        {
            if (User == null ||
                ...
                !User.IsInRole(GXRoles.UserGroup) &&
                !User.IsInRole(GXRoles.UserGroupManager)))
            {
                throw new UnauthorizedAccessException();
            }
            GXSelectArgs args = GXQuery.GetUsersByUserGroup(s => s.Id,
        ...
        /// <inheritdoc />
        public async Task<List<string>> GetUsersAsync(IEnumerable<Guid>? groupId)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.UserGroup) &&
            ...
            bool delete)
        {
            ...
            if (performanceSettings != null)
            {
                ...
                {
                    var users = NotificationRepository.GetNotifiedUsers(_host.Connection, performanceSettings,
                        it.Value, TargetType.UserGroup, NotificationAction.Remove);
                    if (users == null)
                    {
                        break;
                    }
                    GXUserGroup tmp = new GXUserGroup() { Id = it.Key.Id, Name = it.Key.Name };
                    await _eventsNotifier.UserGroupDelete(users, [tmp]);
                }
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (allUsers)
            {
                ...
                {
                    arg.Where.And<GXUserGroup>(w => !request.Exclude.Contains(w.Id));
                }
                if (request?.Included != null && request.Included.Any())
                {
                    arg.Where.And<GXUserGroup>(w => request.Included.Contains(w.Id));
                }
            ...
            else
            {
                ...
                arg = GXQuery.GetUserGroupsByUser(s => s.Id, userId, null,
        ...
        /// <inheritdoc />
        public async Task<GXUserGroup> ReadAsync(Guid id)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.UserGroup) &&
            ...
            {
                string userId = ServerHelpers.GetUserId(User);
                arg = GXQuery.GetUserGroupsByUser(s => s.Id, userId, id);
                arg.Columns.Clear();
                arg.Columns.Add<GXUserGroup>();
            }
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXUserGroup, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXUserGroup>(w => w.Id == id);
            ...
            Expression<Func<GXUserGroup, object?>>? columns)
        {
            ...
            try
            {
                ...
                foreach (GXUserGroup userGroup in updatedGroups)
                {
                    ...
                    List<string> users = await GetUsersAsync(userGroup.Id);
                ...
                foreach (GXUserGroup userGroup in userGroups)
                {
                    ...
                    if (userGroup.DeviceGroups != null && userGroup.DeviceGroups.Any())
                    {
                        ...
                        {
                            RemoveDeviceGroupsFromUserGroup(transaction, userGroup.Id, removedDeviceGroups);
                        }
                        if (addedDeviceGroups.Any())
                        {
                            AddDeviceGroupToUserGroup(transaction, userGroup.Id, addedDeviceGroups);
                        }
                    }
                    ...
                    if (userGroup.DeviceTemplateGroups != null && userGroup.DeviceTemplateGroups.Any())
                    {
                        ...
                        if (addedGroups.Any())
                        {
                            AddDeviceTemplateGroupToUserGroup(transaction, userGroup.Id, addedGroups);
                        }
                    }
                    ...
                    if (userGroup.ScheduleGroups != null && userGroup.ScheduleGroups.Any())
                    {
                        ...
                        {
                            RemoveScheduleGroupsFromUserGroup(transaction, userGroup.Id, removedScheduleGroups);
                        }
                        if (addedScheduleGroups.Any())
                        {
                            AddScheduleGroupToUserGroup(transaction, userGroup.Id, addedScheduleGroups);
                        }
                    }
                }
                _host.Connection.CommitTransaction(transaction);
        ...
        public void AddUsersToGroup(IDbTransaction transaction, IEnumerable<string> users, Guid groupId)
        {
            DateTime now = DateTime.Now;
        ...
        {
            AddUserToGroups(null, userId, groups);
        }
        ...
        /// <param name="userId">User ID.</param>
        /// <param name="groups">Group IDs of the user groups where the user is added.</param>
        public void AddUserToGroups(IDbTransaction? transaction, string userId, IEnumerable<Guid> groups)
        {
            DateTime now = DateTime.Now;
            List<GXUserGroupUser> list = new List<GXUserGroupUser>();
        ...
        /// <inheritdoc />
        public async Task<List<GXUserGroup>> GetDefaultUserGroups(string userId)
        {
            GXSelectArgs args = GXQuery.GetUserGroupsByUser(s => s.Id, userId);
