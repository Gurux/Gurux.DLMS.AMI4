...
namespace Gurux.DLMS.AMI.Server.Repository
{
    ...
    public class ComponentViewGroupRepository : IComponentViewGroupRepository
    {
        ...
        /// <inheritdoc />
        public async Task<List<GXComponentViewGroup>> GetJoinedComponentViewGroups(Guid moduleId)
        {
        ...
        private async Task<List<GXComponentView>> GetJoinedComponentViews(Guid componentViewGroupId)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.ComponentViewGroupManager)))
            {
        ...
        public async Task<List<string>> GetUsersAsync(Guid? groupId)
        {
            ...
            GXSelectArgs args = GXQuery.GetUsersByComponentViewGroup(s => s.Id, 
        ...
        public async Task<List<string>> GetUsersAsync(IEnumerable<Guid>? groupIds)
        {
            ...
            GXSelectArgs args = GXQuery.GetUsersByComponentViewGroups(s => s.Id, 
        ...
        public async Task DeleteAsync(IEnumerable<Guid> userGrouprs, bool delete)
        {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.ComponentViewGroup, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXComponentViewGroup tmp = new GXComponentViewGroup() { Id = it.Key.Id, Name = it.Key.Name };
                await _eventsNotifier.ComponentViewGroupDelete(users, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetComponentViewGroupsByUser(s => "*", userId, null);
            }
            if (request != null)
            {
                ...
                {
                    arg.Where.And<GXComponentViewGroup>(w => !request.Exclude.Contains(w.Id));
                }
                if (request?.Included != null && request.Included.Any())
                {
                    arg.Where.And<GXComponentViewGroup>(w => request.Included.Contains(w.Id));
                }
            }
        ...
        /// <inheritdoc />
        public async Task<GXComponentViewGroup> ReadAsync(Guid id)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.ComponentViewGroupManager)))
            ...
            {
                string userId = ServerHelpers.GetUserId(User);
                arg = GXQuery.GetComponentViewGroupsByUser(s => "*", userId, id);
            }
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXComponentViewGroup, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXComponentViewGroup>(w => w.Id == id);
            ...
            Expression<Func<GXComponentViewGroup, object?>>? columns)
        {
            ...
            foreach (GXComponentViewGroup it in ComponentViewGroups)
            {
                ...
                else
                {
                    ...
                    {
                        List<GXComponentView> list3 = await GetJoinedComponentViews(it.Id);
