...
namespace Gurux.DLMS.AMI.Server.Repository
{
    ...
    public class KeyManagementGroupRepository : IKeyManagementGroupRepository
    {
        ...
        private async Task<List<GXKeyManagement>> GetJoinedKeyManagements(Guid groupId)
        {
        ...
        /// <inheritdoc />
        public async Task<List<GXKeyManagementGroup>> GetJoinedKeyManagementGroups(Guid keyId)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.KeyManagementGroup) &&
                !User.IsInRole(GXRoles.KeyManagementGroupManager)))
            {
        ...
        public async Task<List<string>> GetUsersAsync(Guid? groupId)
        {
            ...
            GXSelectArgs args = GXQuery.GetUsersByKeyManagementGroup(s => s.Id, 
        ...
        public async Task<List<string>> GetUsersAsync(IEnumerable<Guid>? groupIds)
        {
            ...
            GXSelectArgs args = GXQuery.GetUsersByKeyManagementGroups(s => s.Id, 
            ...
            bool delete)
        {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.KeyManagementGroup, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXKeyManagementGroup tmp = new GXKeyManagementGroup() { Id = it.Key.Id, Name = it.Key.Name };
                await _eventsNotifier.KeyManagementGroupDelete(users, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetKeyManagementGroupsByUser(s => "*", 
                    userId, null);
            }
            if (request != null)
            {
        ...
        /// <inheritdoc />
        public async Task<GXKeyManagementGroup> ReadAsync(Guid id)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.KeyManagementGroup) &&
            ...
            {
                string userId = ServerHelpers.GetUserId(User);
                arg = GXQuery.GetKeyManagementGroupsByUser(s => "*", userId, id);
            }
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXKeyManagementGroup, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXKeyManagementGroup>(w => w.Id == id);
            ...
            Expression<Func<GXKeyManagementGroup, object?>>? columns)
        {
            ...
            foreach (GXKeyManagementGroup it in KeyManagementGroups)
            {
                ...
                else
                {
                    ...
                    if (it.KeyManagements != null)
                    {
                        List<GXKeyManagement> list3 = await GetJoinedKeyManagements(it.Id);
            ...
            foreach (var it in updates)
            {
                ...
                await _eventsNotifier.KeyManagementGroupUpdate(it.Value,
                    [new GXKeyManagementGroup() { Id = it.Key.Id, Name = it.Key.Name }]);
            }
            return list.ToArray();
        }
