...
namespace Gurux.DLMS.AMI.Server.Repository
{
    ...
    public class DeviceTemplateGroupRepository : IDeviceTemplateGroupRepository
    {
        ...
        /// <inheritdoc />
        public async Task<List<GXDeviceTemplateGroup>> GetJoinedDeviceTemplateGroups(Guid deviceTemplateId)
        {
        ...
        /// <param name="groupId">Device template group ID.</param>
        /// <returns>User groups where device template belongs.</returns>
        private async Task<List<GXDeviceTemplate>> GetJoinedDeviceTemplate(Guid groupId)
        {
        ...
        {
            GXSelectArgs args = GXQuery.GetUsersByDeviceTemplateGroup(s => s.Id,
        ...
        {
            GXSelectArgs args = GXQuery.GetUsersByDeviceTemplateGroups(s => s.Id,
            ...
            bool delete)
        {
            if (User == null ||
                ...
                !User.IsInRole(GXRoles.DeviceTemplateGroup) &&
                !User.HasScope(GXDeviceTemplateGroupPolicies.Delete)))
            {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.DeviceTemplateGroup, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                await _eventsNotifier.DeviceTemplateGroupDelete(users, [it.Key]);
            }
        }
        /// <inheritdoc />
            ...
            CancellationToken cancellationToken)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.DeviceTemplateGroup) &&
                !User.HasScope(GXDeviceTemplateGroupPolicies.View)))
            {
                throw new UnauthorizedAccessException();
            }
            ...
            else
            {
                ...
                arg = GXQuery.GetDeviceTemplateGroupsByUser(s => "*", userId, null,
        ...
        /// <inheritdoc />
        public async Task<GXDeviceTemplateGroup> ReadAsync(Guid id)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.DeviceTemplateGroup) &&
            ...
            {
                string userId = ServerHelpers.GetUserId(User);
                arg = GXQuery.GetDeviceTemplateGroupsByUser(s => "*", userId, id);
            }
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXDeviceTemplateGroup, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXDeviceTemplateGroup>(w => w.Id == id);
            ...
            Expression<Func<GXDeviceTemplateGroup, object?>>? columns)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.DeviceTemplateGroup) &&
                !User.HasScope(GXDeviceTemplateGroupPolicies.Edit)))
            {
                throw new UnauthorizedAccessException();
            }
            DateTime now = DateTime.Now;
            ...
            try
            {
                ...
                foreach (var it in updatedGroups)
                {
                    ...
                    {
                        List<GXDeviceTemplate> list2 = await GetJoinedDeviceTemplate(it.Id);
                        List<Guid> groups = list2.Select(s => s.Id).ToList();
