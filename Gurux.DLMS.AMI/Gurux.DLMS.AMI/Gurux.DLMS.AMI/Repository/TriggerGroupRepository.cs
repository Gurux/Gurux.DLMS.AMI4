    ...
    public class TriggerGroupRepository : ITriggerGroupRepository
    {
            ...
            GXPerformanceSettings performanceSettings)
        {
            ...
            if (user == null ||
                ...
                !user.IsInRole(GXRoles.TriggerGroup) &&
                !user.IsInRole(GXRoles.TriggerGroupManager)))
            {
        ...
        /// <inheritdoc />
        public async Task<List<GXTriggerGroup>> GetJoinedTriggerGroups(Guid triggerId)
        {
        ...
        {
            GXSelectArgs args = GXQuery.GetUsersByTriggerGroup(s => s.Id,
        ...
        {
            GXSelectArgs args = GXQuery.GetUsersByTriggerGroups(s => s.Id,
            ...
            bool delete)
        {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.TriggerGroup, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXTriggerGroup tmp = new GXTriggerGroup() { Id = it.Key.Id };
                await _eventsNotifier.TriggerGroupDelete(users, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetTriggerGroupsByUser(s => s.Id, userId);
            }
            if (request != null)
            {
        ...
        /// <inheritdoc />
        public async Task<GXTriggerGroup> ReadAsync(Guid id)
        {
            GXSelectArgs arg;
            if (User.IsInRole(GXRoles.Admin))
            {
            ...
            {
                string userId = ServerHelpers.GetUserId(User);
                arg = GXQuery.GetTriggerGroupsByUser(s => s.Id, userId, id);
            }
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXTriggerGroup, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXTriggerGroup>(w => w.Id == id);
