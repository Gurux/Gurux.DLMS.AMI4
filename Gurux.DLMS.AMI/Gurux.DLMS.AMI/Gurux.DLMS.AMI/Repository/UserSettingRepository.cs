...
namespace Gurux.DLMS.AMI.Server.Repository
{
    ...
    public class UserSettingRepository : IUserSettingRepository
    {
        ...
        /// <inheritdoc />
        public async Task DeleteAsync(IEnumerable<string> settings)
        {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (request != null)
            {
                ...
                {
                    arg.Where.And<GXUserSetting>(w => !request.Exclude.Contains(w.Id));
                }
                if (request?.Included != null && request.Included.Any())
                {
                    arg.Where.And<GXUserSetting>(w => request.Included.Contains(w.Id));
                }
            }
        ...
        /// <inheritdoc />
        public async Task<GXUserSetting> ReadAsync(string name)
        {
            GXUser user = new GXUser() { Id = ServerHelpers.GetUserId(User) };
            ...
            Expression<Func<GXUserSetting, object?>>? columns)
        {
            ...
            foreach (GXUserSetting it in settings)
            {
                ...
                GXUserSetting ret = await ReadAsync(it.Name);
                if (ret == null)
                {
                    it.CreationTime = now;
                    it.User = user;
