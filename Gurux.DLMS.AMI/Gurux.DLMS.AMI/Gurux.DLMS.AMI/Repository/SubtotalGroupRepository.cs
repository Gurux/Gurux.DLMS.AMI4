    ...
    public class SubtotalGroupRepository : ISubtotalGroupRepository
    {
            ...
            GXPerformanceSettings performanceSettings)
        {
            ...
            if (user == null ||
                ...
                !user.IsInRole(GXRoles.SubtotalGroup) &&
                !user.IsInRole(GXRoles.SubtotalGroupManager)))
            {
        ...
        /// <inheritdoc />
        public async Task<List<GXSubtotalGroup>> GetJoinedSubtotalGroups(Guid moduleId)
        {
        ...
        private async Task<List<GXSubtotal>> GetJoinedSubtotals(Guid subtotalGroupId)
        {
        ...
        public async Task DeleteAsync(IEnumerable<Guid> userGrouprs, bool delete)
        {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.SubtotalGroup, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXSubtotalGroup tmp = new GXSubtotalGroup() { Id = it.Key.Id, Name = it.Key.Name };
                await _eventsNotifier.SubtotalGroupDelete(users, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetSubtotalGroupsByUser(s => "*", userId);
            }
            if (request != null)
            {
        ...
        /// <inheritdoc />
        public async Task<GXSubtotalGroup> ReadAsync(Guid id)
        {
            GXSelectArgs arg;
            if (User.IsInRole(GXRoles.Admin))
            {
            ...
            {
                string userId = ServerHelpers.GetUserId(User);
                arg = GXQuery.GetSubtotalGroupsByUser(s => s.Id, userId, id);
            }
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXSubtotalGroup, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXSubtotalGroup>(w => w.Id == id);
            ...
            Expression<Func<GXSubtotalGroup, object?>>? columns)
        {
            ...
            foreach (GXSubtotalGroup it in SubtotalGroups)
            {
                ...
                if (it.UserGroups == null || !it.UserGroups.Any())
                {
                    ...
                    it.UserGroups = await _userGroupRepository.GetDefaultUserGroups(creator.Id);
                    if (it.UserGroups == null || !it.UserGroups.Any())
                    {
                ...
                else
                {
                    ...
                    {
                        List<GXSubtotal> list3 = await GetJoinedSubtotals(it.Id);
