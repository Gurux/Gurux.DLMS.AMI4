...
namespace Gurux.DLMS.AMI.Server.Repository
{
    ...
    public class WorkflowGroupRepository : IWorkflowGroupRepository
    {
        ...
        /// <inheritdoc />
        public async Task<List<GXWorkflowGroup>> GetJoinedWorkflowGroups(Guid workflowId)
        {
        ...
        public async Task<List<string>> GetUsersAsync(Guid? groupId)
        {
            if (User == null ||
                ...
                !User.IsInRole(GXRoles.WorkflowGroup) &&
                !User.IsInRole(GXRoles.WorkflowGroupManager)))
            {
                throw new UnauthorizedAccessException();
            }
            GXSelectArgs args = GXQuery.GetUsersByWorkflowGroup(s => s.Id, 
        ...
        public async Task<List<string>> GetUsersAsync(IEnumerable<Guid>? groupIds)
        {
            ...
            GXSelectArgs args = GXQuery.GetUsersByWorkflowGroups(s => s.Id, 
            ...
            bool delete)
        {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.WorkflowGroup, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXWorkflowGroup tmp = new GXWorkflowGroup() { Id = it.Key.Id, Name = it.Key.Name };
                await _eventsNotifier.WorkflowGroupDelete(users, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetWorkflowGroupsByUser(s => "*", userId);
            }
            if (request != null)
            {
                //If workflow groups are filtered by user.
                if (request.Filter?.UserGroups != null)
                {
                    ...
                    if (ug?.Users != null && ug.Users.Any())
                    {
                        ...
                        if (user != null)
                        {
                            arg.Joins.AddLeftJoin<GXUserGroupWorkflowGroup, GXUserGroup>(j => j.UserGroupId, j => j.Id);
                ...
                {
                    arg.Where.And<GXWorkflowGroup>(w => !request.Exclude.Contains(w.Id));
                }
                if (request?.Included != null && request.Included.Any())
                {
                    arg.Where.And<GXWorkflowGroup>(w => request.Included.Contains(w.Id));
                }
            }
        ...
        /// <inheritdoc />
        public async Task<GXWorkflowGroup> ReadAsync(Guid id)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.WorkflowGroup) &&
            ...
            {
                string userId = ServerHelpers.GetUserId(User);
                arg = GXQuery.GetWorkflowGroupsByUser(s => s.Id, userId);
            }
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXWorkflowGroup, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXWorkflowGroup>(w => w.Id == id);
            ...
            Expression<Func<GXWorkflowGroup, object?>>? columns)
        {
            ...
            foreach (var it in updates)
            {
                await _eventsNotifier.WorkflowGroupUpdate(it.Value, [it.Key]);
            }
            return list.ToArray();
        }
