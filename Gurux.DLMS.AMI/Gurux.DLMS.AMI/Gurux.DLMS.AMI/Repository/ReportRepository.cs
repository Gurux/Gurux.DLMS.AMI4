    ...
    public class ReportRepository : IReportRepository
    {
            ...
            GXPerformanceSettings performanceSettings)
        {
            ...
            if (user == null ||
                ...
                !user.IsInRole(GXRoles.Report) &&
                !user.IsInRole(GXRoles.ReportManager)))
            {
        ...
        /// <inheritdoc />
        public async Task<List<string>> GetUsersAsync(Guid? reportId)
        {
            GXSelectArgs args = GXQuery.GetUsersByReport(s => s.Id,
                ServerHelpers.GetUserId(User), reportId);
        ...
        /// <inheritdoc />
        public async Task<List<string>> GetUsersAsync(IEnumerable<Guid>? reportIds)
        {
            GXSelectArgs args = GXQuery.GetUsersByReports(s => s.Id,
                ServerHelpers.GetUserId(User), reportIds);
            List<GXUser> users = await _host.Connection.SelectAsync<GXUser>(args);
            List<string> ret = users.Select(s => s.Id).ToList();
            if (User.IsInRole(GXRoles.Admin))
            {
            ...
            bool delete)
        {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.Report, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXReport tmp = new GXReport() { Id = it.Key.Id };
                await _eventsNotifier.ReportDelete(users, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetReportsByUser(s => "*", userId);
            }
            if (request != null)
            {
        ...
        /// <inheritdoc />
        public async Task<GXReport> ReadAsync(Guid id)
        {
            string userId = ServerHelpers.GetUserId(User);
            GXSelectArgs arg;
            if (User.IsInRole(GXRoles.Admin))
            ...
            {
                arg = GXQuery.GetReportsByUser(s => s.Id, userId, id);
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXReport, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXReport>(w => w.Id == id);
            report.Creator = await _host.Connection.SingleOrDefaultAsync<GXUser>(arg);
            return report;
        }
            ...
            Expression<Func<GXReport, object?>>? columns)
        {
            ...
            try
            {
                foreach (GXReport report in reports)
                {
                    ...
                    if (report.ReportGroups == null || !report.ReportGroups.Any())
                    {
                        ...
                        report.ReportGroups.AddRange(await _reportGroupRepository.ListAsync(request, null, CancellationToken.None));
                        ...
                        {
                            {
                                ...
                                reportGroups = await reportGroupRepository.GetJoinedReportGroups(report.Id);
            ...
            foreach (GXReport report in reports)
            {
                updates[report] = await GetUsersAsync(report.Id);
            }
            foreach (var it in updates)
            {
        ...
        /// <inheritdoc />
        public async Task<IEnumerable<GXReport>> SendAsync(IEnumerable<GXReport> reports)
        {
            List<GXReport> list = new List<GXReport>();
            foreach (var report in reports)
            {
                var tmp = await ReadAsync(report.Id);
        ...
        /// <inheritdoc />
        public async Task CancelAsync(IEnumerable<Guid>? reports)
        {
            if (reports != null)
            {
                Dictionary<GXReport, List<string>> updates = new();
                ...
                foreach (var report in list)
                {
                    if (report.Status == ReportStatus.Process)
                    {
                        ...
                        await UpdateAsync([report], c => new { c.Status });
                        List<string> users = await GetUsersAsync(report.Id);
                ...
                foreach (var it in updates)
                {
                    ...
                    if (it.Key.Status == ReportStatus.Cancel)
                    {
                        ...
                        await _eventsNotifier.ReportClear(it.Value, [tmp]);
                    }
                }
            }
        }
