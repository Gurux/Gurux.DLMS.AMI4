...
namespace Gurux.DLMS.AMI.Server.Repository
{
    ...
    public class GatewayRepository : IGatewayRepository
    {
        ...
        /// <inheritdoc />
        public async Task<List<string>> GetUsersAsync(Guid? gatewayId)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.Gateway) &&
                !User.IsInRole(GXRoles.GatewayManager) &&
                !User.HasScope(GXGatewayPolicies.View)))
            {
                throw new UnauthorizedAccessException();
            }
            GXSelectArgs args = GXQuery.GetUsersByGateway(s => s.Id,
                ServerHelpers.GetUserId(User), gatewayId);
        ...
        /// <inheritdoc />
        public async Task<List<string>> GetUsersAsync(IEnumerable<Guid>? gatewayIds)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.Gateway) &&
            ...
            GXSelectArgs args = GXQuery.GetUsersByGateways(s => s.Id,
                ServerHelpers.GetUserId(User), gatewayIds);
            List<GXUser> users = await _host.Connection.SelectAsync<GXUser>(args);
            List<string> ret = users.Select(s => s.Id).ToList();
            if (User.IsInRole(GXRoles.Admin))
            {
        ...
        private async Task<List<GXGatewayGroup>> GetGatewayGroupsByGatewayId(Guid gatewayId)
        {
        ...
        private async Task<List<GXDeviceGroup>> GetDeviceGroupsByGatewayId(Guid gatewayId)
        {
        ...
        private async Task<List<GXDevice>> GetDevicesByGatewayId(Guid gatewayId)
        {
            ...
            bool delete)
        {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.Gateway, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXGateway tmp = new GXGateway() { Id = it.Key.Id };
                await _eventsNotifier.GatewayDelete(users, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetGatewaysByUser(s => "*", userId);
            }
            if (request != null)
            {
        ...
        /// <inheritdoc />
        public async Task<GXGateway> ReadAsync(Guid id)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.Gateway) &&
            ...
            //Get groups.
            gateway.GatewayGroups = await GetGatewayGroupsByGatewayId(gateway.Id);
            ////////////////////////////////////////////////////
            //Get device groups.
            gateway.DeviceGroups = await GetDeviceGroupsByGatewayId(id);
            ////////////////////////////////////////////////////
            //Get agent.
            ...
            arg.Where.And<GXGateway>(q => q.Removed == null && q.Id == id);
            gateway.Agent = await _host.Connection.SingleOrDefaultAsync<GXAgent>(arg);

            ////////////////////////////////////////////////////
            //Get devices.
            gateway.Devices = await GetDevicesByGatewayId(id);
            //Get creator with own query. It's faster for some DBs.
            arg = GXSelectArgs.Select<GXUser>(s => new { s.Id, s.UserName }, q => q.Removed == null);
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXGateway, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXGateway>(w => w.Id == id);
            gateway.Creator = await _host.Connection.SingleOrDefaultAsync<GXUser>(arg);
            return gateway;
        }
            ...
            Expression<Func<GXGateway, object?>>? columns)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.GatewayManager) &&
                !User.HasScope(GXGatewayPolicies.Add) &&
                !User.HasScope(GXGatewayPolicies.Edit)))
            {
                throw new UnauthorizedAccessException();
            }
            ...
            foreach (GXGateway gateway in gateways)
            {
                ...
                if (gateway.Id == Guid.Empty)
                {
                    if (gateway.GatewayGroups == null || !gateway.GatewayGroups.Any())
                    {
                        ...
                        GXGatewayGroup[] groups = await _gatewayGroupRepository.ListAsync(request, null, CancellationToken.None);
                ...
                else
                {
                    ...
                    if (gateway.GatewayGroups != null && gateway.GatewayGroups.Any())
                    {
                        List<GXGatewayGroup> gatewayGroups = await GetGatewayGroupsByGatewayId(gateway.Id);
                    ...
                    {
                        List<GXDeviceGroup> deviceGroups = await GetDeviceGroupsByGatewayId(gateway.Id);
                    ...
                    {
                        List<GXDevice> devices = await GetDevicesByGatewayId(gateway.Id);
                ...
                updates[gateway] = await GetUsersAsync(gateway.Id);
            }
            if (_performanceSettings.Notification(TargetType.Gateway))
            {
                foreach (var it in updates)
                {
        ...
        /// <inheritdoc />
        public async Task UpdateStatusAsync(Guid gatewayId, GatewayStatus status)
        {
            ...
            if (_performanceSettings.Notification(TargetType.Gateway))
            {
                await _eventsNotifier.GatewayStatusChange(await GetUsersAsync(gateway.Id), new GXGateway[] { tmp });
            }
            ...
            if (log != null)
            {
                ...
                await repository.AddAsync(status.ToString(), [log]);
            }
        }
        ...
        /// <inheritdoc />
        public Task ResetAsync(IEnumerable<Guid> gateways)
        {
