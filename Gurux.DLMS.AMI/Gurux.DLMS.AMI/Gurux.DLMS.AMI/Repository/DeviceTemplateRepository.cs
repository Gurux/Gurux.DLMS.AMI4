...
namespace Gurux.DLMS.AMI.Server.Repository
{
    ...
    public class DeviceTemplateRepository : IDeviceTemplateRepository
    {
        ...
        /// <inheritdoc />
        public async Task<List<string>> GetUsersAsync(Guid? templateId)
        {
            GXSelectArgs args = GXQuery.GetUsersByDeviceTemplate(s => s.Id,
                ServerHelpers.GetUserId(User), templateId);
        ...
        /// <inheritdoc />
        public async Task<List<string>> GetUsersAsync(IEnumerable<Guid>? templateIds)
        {
            GXSelectArgs args = GXQuery.GetUsersByDeviceTemplates(s => s.Id,
                ServerHelpers.GetUserId(User), templateIds);
            List<GXUser> users = await _host.Connection.SelectAsync<GXUser>(args);
            List<string> ret = users.Select(s => s.Id).ToList();
            if (User != null && User.IsInRole(GXRoles.Admin))
            {
        ...
        /// <inheritdoc />
        public async Task DeleteAsync(IEnumerable<Guid> deviceTemplates,
            bool delete)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.DeviceTemplateManager) &&
                !User.HasScope(GXDeviceTemplatePolicies.Edit)))
            {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.DeviceTemplate, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                await _eventsNotifier.DeviceTemplateDelete(users, [it.Key]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.DeviceTemplateManager) &&
                !User.HasScope(GXDeviceTemplatePolicies.View)))
            {
                throw new UnauthorizedAccessException();
            }
            ...
            else
            {
                ...
                arg = GXQuery.GetDeviceTemplatesByUser(s => "*", userId, null,
        ...
        /// <inheritdoc />
        public async Task<GXDeviceTemplate> ReadAsync(Guid id)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.DeviceTemplateManager) &&
            ...
            {
                string userId = ServerHelpers.GetUserId(User);
                arg = GXQuery.GetDeviceTemplatesByUser(s => "*", userId, id);
            }
            ...
            if (!string.IsNullOrEmpty(ret.Settings))
            {
                ...
                if (!string.IsNullOrEmpty(s.Password))
                {
                    try
                    {
                        s.Password = await _cryproService.DecryptAsync(s.Password);
                    }
                    catch (Exception)
                    {
                ...
                if (s.HexPassword != null && s.HexPassword.Any())
                {
                    try
                    {
                        s.HexPassword = await _cryproService.DecryptAsync(s.HexPassword);
                    }
                    catch (Exception)
                    {
                        //Old data is not encrypted.
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXDeviceTemplate, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXDeviceTemplate>(w => w.Id == id);
            ret.Creator = await _host.Connection.SingleOrDefaultAsync<GXUser>(arg);
            return ret;
        }
        ...
        private void EncryptPassword(GXDeviceTemplate it)
        {
            if (!string.IsNullOrEmpty(it.Settings))
            {
                ...
                if (!string.IsNullOrEmpty(s.Password))
                {
                    s.Password = _cryproService.EncryptAsync(s.Password).Result;
                }
                if (s.HexPassword != null && s.HexPassword.Any())
                {
                    s.HexPassword = _cryproService.EncryptAsync(s.HexPassword).Result;
                }
            ...
            Expression<Func<GXDeviceTemplate, object?>>? columns)
        {
            ...
            foreach (GXDeviceTemplate it in DeviceTemplates)
            {
                ...
                if (it.DeviceTemplateGroups == null || !it.DeviceTemplateGroups.Any())
                {
                    if (defaultGroups == null)
                    {
                        ...
                        defaultGroups = new List<GXDeviceTemplateGroup>(await _deviceTemplateGroupRepository.ListAsync(
            ...
            try
            {
                if (newTemplates.Any())
                {
                    ...
                    //Map device templates to device template groups.
                    foreach (var dt in newTemplates)
                    {
                        AddDeviceTemplateToDeviceTemplateGroups(transaction, dt.Id, dt.DeviceTemplateGroups);
                    }
                ...
                foreach (var it in updatedTemplates)
                {
                    ...
                    using (IServiceScope scope = _serviceProvider.CreateScope())
                    {
                        ...
                        deviceTemplateGroups = await deviceTemplateGroupRepository.GetJoinedDeviceTemplateGroups(it.Id);
                    }
