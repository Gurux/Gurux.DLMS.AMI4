    ...
    class ReportLogRepository : IReportLogRepository
    {
            ...
            GXPerformanceSettings performanceSettings)
        {
            ...
            if (user == null ||
                ...
                !user.IsInRole(GXRoles.ReportLog) &&
                !user.IsInRole(GXRoles.ReportLogManager)))
            {
        ...
        /// <inheritdoc />
        public async Task AddAsync(string type, IEnumerable<GXReportLog> errors)
        {
            ...
            {
                it.CreationTime = now;
                it.Type = await _enumTypeRepository.GetLogTypeAsync(TargetType.ReportLog, type);
                updates[it] = await _reportRepository.GetUsersAsync(it.Report.Id);
            }
            ...
            foreach (var it in updates)
            {
                ...
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                                  it.Value, TargetType.ReportLog, NotificationAction.Add);
                if (users != null && users.Any())
                {
                    await _eventsNotifier.AddReportLogs(users, [tmp]);
                }
            }
        }

        /// <inheritdoc />
        public async Task<GXReportLog> AddAsync(string type, GXReport report, Exception ex)
        {
            DateTime now = DateTime.Now;
            Dictionary<GXReportLog, List<string>> updates = new Dictionary<GXReportLog, List<string>>();
            GXReportLog error = new GXReportLog(TraceLevel.Error)
            {
                ...
                Message = ex.Message,
                Report = report,
                Type = await _enumTypeRepository.GetLogTypeAsync(TargetType.ReportLog, type)
            };
            await _host.Connection.InsertAsync(GXInsertArgs.Insert(error));
            updates[error] = await _reportRepository.GetUsersAsync(report.Id);
            foreach (var it in updates)
            {
        ...
        /// <inheritdoc />
        public async Task ClearAsync(Guid[]? reports)
        {
            ...
            if (!admin)
            {
                ...
                arg = GXQuery.GetReportsByUser(s => s.Id, id);
            }
            else
            {
            ...
            users.AddDistinct(await _reportRepository.GetUsersAsync(reports));
            ...
            users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                users, TargetType.ReportLog, NotificationAction.Clear);
            if (users != null && users.Any())
            {
                await _eventsNotifier.ClearReportLogs(users, logs);
            }
            await AddAsync(TargetType.ReportLog, [new GXReportLog(TraceLevel.Info)
            {
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetReportErrorsByUser(s => s.Id, userId);
            }
            if (request != null)
            {
        ...
        /// <inheritdoc />
        public async Task<GXReportLog> ReadAsync(Guid id)
        {
