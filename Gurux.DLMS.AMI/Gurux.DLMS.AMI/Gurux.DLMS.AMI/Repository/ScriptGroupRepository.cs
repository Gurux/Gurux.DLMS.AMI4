...
namespace Gurux.DLMS.AMI.Server.Repository
{
    ...
    public class ScriptGroupRepository : IScriptGroupRepository
    {
        ...
        /// <inheritdoc />
        public async Task<List<GXScriptGroup>> GetJoinedScriptGroups(Guid scriptId)
        {
        ...
        public async Task<List<string>> GetUsersAsync(Guid? groupId)
        {
            if (User == null ||
                ...
                !User.IsInRole(GXRoles.ScriptGroup) &&
                !User.IsInRole(GXRoles.ScriptGroupManager)))
            {
                throw new UnauthorizedAccessException();
            }
            GXSelectArgs args = GXQuery.GetUsersByScriptGroup(s => s.Id,
        ...
        public async Task<List<string>> GetUsersAsync(IEnumerable<Guid>? groupIds)
        {
            ...
            GXSelectArgs args = GXQuery.GetUsersByScriptGroups(s => s.Id,
            ...
            bool delete)
        {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.ScriptGroup, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXScriptGroup tmp = new GXScriptGroup() { Id = it.Key.Id, Name = it.Key.Name };
                await _eventsNotifier.ScriptGroupDelete(users, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetScriptGroupsByUser(s => "*", userId);
            }
            if (request != null)
            {
                ...
                {
                    arg.Where.And<GXScriptGroup>(w => request.Included.Contains(w.Id));
                }
            }
            arg.Distinct = true;
        ...
        /// <inheritdoc />
        public async Task<GXScriptGroup> ReadAsync(Guid id)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.ScriptGroup) &&
            ...
            else
            {
                ...
                arg = GXQuery.GetScriptGroupsByUser(s => s.Id, userId, id);
            }
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXScriptGroup, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXScriptGroup>(w => w.Id == id);
