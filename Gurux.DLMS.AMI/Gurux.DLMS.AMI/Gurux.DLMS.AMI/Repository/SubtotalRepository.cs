    ...
    public class SubtotalRepository : ISubtotalRepository
    {
            ...
            GXPerformanceSettings performanceSettings)
        {
            ...
            if (user == null ||
                ...
                !user.IsInRole(GXRoles.Subtotal) &&
                !user.IsInRole(GXRoles.SubtotalManager)))
            {
        ...
        /// <inheritdoc />
        public async Task<List<string>> GetUsersAsync(Guid? subtotalId)
        {
        ...
        /// <inheritdoc />
        public async Task<List<string>> GetUsersAsync(IEnumerable<Guid>? subtotalIds)
        {
        ...
        /// <inheritdoc />
        public async Task DeleteAsync(IEnumerable<Guid> subtotals, bool delete)
        {
            if (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.SubtotalManager))
            {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.Subtotal, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXSubtotal tmp = new GXSubtotal() { Id = it.Key.Id };
                await _eventsNotifier.SubtotalDelete(users, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetSubtotalsByUser(s => "*", userId);
            }
            if (request != null)
            {
        ...
        /// <inheritdoc />
        public async Task<GXSubtotal> ReadAsync(Guid id)
        {
            string userId = ServerHelpers.GetUserId(User);
            GXSelectArgs arg;
            if (User.IsInRole(GXRoles.Admin))
            ...
            {
                arg = GXQuery.GetSubtotalsByUser(s => s.Id, userId, id);
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXSubtotal, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXSubtotal>(w => w.Id == id);
            subtotal.Creator = await _host.Connection.SingleOrDefaultAsync<GXUser>(arg);
            return subtotal;
        }
            ...
            Expression<Func<GXSubtotal, object?>>? columns)
        {
            ...
            try
            {
                foreach (GXSubtotal subtotal in subtotals)
                {
                    ...
                    if (subtotal.SubtotalGroups == null || !subtotal.SubtotalGroups.Any())
                    {
                        ...
                        subtotal.SubtotalGroups.AddRange(await _subtotalGroupRepository.ListAsync(request, null, CancellationToken.None));
                        ...
                        {
                            {
                                ...
                                subtotalGroups = await subtotalGroupRepository.GetJoinedSubtotalGroups(subtotal.Id);
            ...
            foreach (GXSubtotal subtotal in subtotals)
            {
                updates[subtotal] = await GetUsersAsync(subtotal.Id);
            }
            foreach (var it in updates)
            {
        ...
        /// <inheritdoc />
        public async Task CalculateAsync(IEnumerable<Guid>? subtotals)
        {
            if (subtotals != null)
            {
                Dictionary<GXSubtotal, List<string>> updates = new();
                ...
                var list = await ListAsync(req, null, CancellationToken.None);
                ...
                foreach (var subtotal in list)
                {
                        ...
                        subtotal.Status != SubtotalStatus.Clear)
                    {
                        ...
                        await UpdateAsync([subtotal], c => c.Status);
                        _subTotal.Calculate(subtotals);
                        List<string> users = await GetUsersAsync(subtotal.Id);
                        updates[subtotal] = users;
                    }
                }
                foreach (var it in updates)
                {
                    //Don't notification if already calulated.
                    if (it.Key.Status == SubtotalStatus.Calculate)
                    {
                        GXSubtotal tmp = new GXSubtotal() { Id = it.Key.Id, Status = it.Key.Status };
                        await _eventsNotifier.SubtotalUpdate(it.Value, [tmp]);
                    }
                }
            }
        }
        ...
        /// <inheritdoc />
        public async Task ClearAsync(IEnumerable<Guid>? subtotals)
        {
            if (subtotals != null)
            {
                Dictionary<GXSubtotal, List<string>> updates = new();
                ...
                foreach (var subtotal in list)
                {
                    subtotal.Last = null;
                    subtotal.Status = SubtotalStatus.Clear;
                    await UpdateAsync([subtotal], c => new { c.Status, c.Last });
                    List<string> users = await GetUsersAsync(subtotal.Id);
                    updates[subtotal] = users;
                }
                foreach (var it in updates)
                {
                    ...
                    {
                        GXSubtotal tmp = new GXSubtotal() { Id = it.Key.Id, Status = it.Key.Status };
                        await _eventsNotifier.SubtotalClear(it.Value, [tmp]);
                    }
                }
                using IDbTransaction transaction = _host.Connection.BeginTransaction();
                foreach (var subtotal in list)
        ...
        /// <inheritdoc />
        public async Task CancelAsync(IEnumerable<Guid>? subtotals)
        {
            if (subtotals != null)
            {
                Dictionary<GXSubtotal, List<string>> updates = new();
                ...
                foreach (var subtotal in list)
                {
                    if (subtotal.Status == SubtotalStatus.Calculate)
                    {
                        _subTotal.Cancel(subtotals);
                        subtotal.Status = SubtotalStatus.Idle;
                        await UpdateAsync([subtotal], c => new { c.Status });
                        List<string> users = await GetUsersAsync(subtotal.Id);
                        updates[subtotal] = users;
                    }
                }
                foreach (var it in updates)
                {
                    //Don't notification if already cancelled.
