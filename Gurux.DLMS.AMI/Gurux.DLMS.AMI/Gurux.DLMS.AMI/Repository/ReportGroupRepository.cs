    ...
    public class ReportGroupRepository : IReportGroupRepository
    {
            ...
            GXPerformanceSettings performanceSettings)
        {
            ...
            if (user == null ||
                ...
                !user.IsInRole(GXRoles.ReportGroup) &&
                !user.IsInRole(GXRoles.ReportGroupManager)))
            {
        ...
        /// <inheritdoc />
        public async Task<List<GXReportGroup>> GetJoinedReportGroups(Guid moduleId)
        {
        ...
        private async Task<List<GXReport>> GetJoinedReports(Guid reportGroupId)
        {
        ...
        {
            GXSelectArgs args = GXQuery.GetUsersByReportGroup(s => s.Id,
        ...
        {
            GXSelectArgs args = GXQuery.GetUsersByReportGroups(s => s.Id, 
            ...
            bool delete)
        {
            ...
            foreach (var it in updates)
            {
                ...
                it.Value, TargetType.ReportGroup, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXReportGroup tmp = new GXReportGroup() { Id = it.Key.Id, Name = it.Key.Name };
                await _eventsNotifier.ReportGroupDelete(users, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetReportGroupsByUser(s => "*", userId);
            }
            if (request != null)
            {
        ...
        /// <inheritdoc />
        public async Task<GXReportGroup> ReadAsync(Guid id)
        {
            GXSelectArgs arg;
            if (User.IsInRole(GXRoles.Admin))
            {
            ...
            {
                string userId = ServerHelpers.GetUserId(User);
                arg = GXQuery.GetReportGroupsByUser(s => s.Id, userId, id);
            }
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXReportGroup, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXReportGroup>(w => w.Id == id);
            ...
            Expression<Func<GXReportGroup, object?>>? columns)
        {
            ...
            foreach (GXReportGroup it in ReportGroups)
            {
                ...
                else
                {
                    ...
                    {
                        List<GXReport> list3 = await GetJoinedReports(it.Id);
            ...
            foreach (var it in updates)
            {
                await _eventsNotifier.ReportGroupUpdate(it.Value, [it.Key]);
            }
            return list.ToArray();
        }
