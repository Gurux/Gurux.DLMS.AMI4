    ...
    public class GatewayGroupRepository : IGatewayGroupRepository
    {
        ...
        public async Task<List<string>> GetUsersAsync(Guid? groupId)
        {
            if (User == null ||
                ...
                !User.IsInRole(GXRoles.GatewayGroup) &&
                !User.IsInRole(GXRoles.GatewayGroupManager)))
            {
                throw new UnauthorizedAccessException();
            }
            GXSelectArgs args = GXQuery.GetUsersByGatewayGroup(s => s.Id,
        ...
        public async Task<List<string>> GetUsersAsync(IEnumerable<Guid>? gatewayIds)
        {
            ...
            GXSelectArgs args = GXQuery.GetUsersByGatewayGroups(s => s.Id,
        ...
        public async Task DeleteAsync(IEnumerable<Guid> userGrouprs, bool delete)
        {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.GatewayGroup, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXGatewayGroup tmp = new GXGatewayGroup() { Id = it.Key.Id, Name = it.Key.Name };
                await _eventsNotifier.GatewayGroupDelete(users, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            if (User == null ||
                ...
                !User.IsInRole(GXRoles.GatewayGroup) &&
                !User.IsInRole(GXRoles.GatewayGroupManager) &&
                !User.HasScope(GXGatewayGroupPolicies.View)))
            {
                throw new UnauthorizedAccessException();
            }
            GXSelectArgs arg;
            ...
            else
            {
                ...
                arg = GXQuery.GetGatewayGroupsByUser(s => "*", userId, null);
            }
            if (request != null)
            {
                ...
                {
                    arg.Where.And<GXGatewayGroup>(w => request.Included.Contains(w.Id));
                }
            }
        ...
        /// <inheritdoc />
        public async Task<GXGatewayGroup> ReadAsync(Guid id)
        {
            if (User == null ||
                (!User.IsInRole(GXRoles.Admin) &&
                !User.IsInRole(GXRoles.GatewayGroup) &&
            ...
            {
                string userId = ServerHelpers.GetUserId(User);
                arg = GXQuery.GetGatewayGroupsByUser(s => "*", userId, id);
            }
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXGatewayGroup, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXGatewayGroup>(w => w.Id == id);
            ...
            Expression<Func<GXGatewayGroup, object?>>? columns)
        {
            ...
            foreach (var it in updates)
            {
                await _eventsNotifier.GatewayGroupUpdate(it.Value, [it.Key]);
            }
            return list.ToArray();
        }
