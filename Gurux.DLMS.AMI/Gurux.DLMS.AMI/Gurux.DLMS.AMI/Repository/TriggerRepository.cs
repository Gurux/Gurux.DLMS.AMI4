    ...
    public class TriggerRepository : ITriggerRepository
    {
            ...
            GXPerformanceSettings performanceSettings)
        {
            ...
            if (user == null ||
                ...
                !user.IsInRole(GXRoles.Trigger) &&
                !user.IsInRole(GXRoles.TriggerManager)))
            {
        ...
        /// <inheritdoc/>
        public async Task<List<string>> GetUsersAsync(Guid? triggerId)
        {
            GXSelectArgs args = GXQuery.GetUsersByTrigger(s => s.Id,
                ServerHelpers.GetUserId(User), triggerId);
        ...
        /// <inheritdoc/>
        public async Task<List<string>> GetUsersAsync(IEnumerable<Guid>? triggerId)
        {
            GXSelectArgs args = GXQuery.GetUsersByTriggers(s => s.Id,
                ServerHelpers.GetUserId(User), triggerId);
            List<GXUser> users = await _host.Connection.SelectAsync<GXUser>(args);
            List<string> ret = users.Select(s => s.Id).ToList();
            if (User != null && User.IsInRole(GXRoles.Admin))
            ...
            bool delete)
        {
            ...
            foreach (var it in updates)
            {
                var users = NotificationRepository.GetNotifiedUsers(_host.Connection, _performanceSettings,
                    it.Value, TargetType.Trigger, NotificationAction.Remove);
                if (users == null)
                {
                    break;
                }
                GXTrigger tmp = new GXTrigger() { Id = it.Key.Id };
                await _eventsNotifier.TriggerDelete(it.Value, [tmp]);
            }
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                arg = GXQuery.GetTriggersByUser(s => s.Id, userId);
            }
            if (request != null)
            {
                ...
                {
                    arg.Where.And<GXTrigger>(w => !request.Exclude.Contains(w.Id));
                }
                if (request?.Included != null && request.Included.Any())
                {
                    arg.Where.And<GXTrigger>(w => request.Included.Contains(w.Id));
                }
            }
            arg.Distinct = true;
        ...
        /// <inheritdoc />
        public async Task<GXTrigger> ReadAsync(Guid id)
        {
            bool isAdmin = false;
            if (User != null)
            {
            ...
            {
                string userId = ServerHelpers.GetUserId(User);
                arg = GXQuery.GetTriggersByUser(s => s.Id, userId, id);
            }
            //Get trigger activities.
            ...
            arg.Distinct = true;
            arg.Joins.AddInnerJoin<GXTrigger, GXUser>(s => s.Creator, o => o.Id);
            arg.Where.And<GXTrigger>(w => w.Id == id);
            trigger.Creator = await _host.Connection.SingleOrDefaultAsync<GXUser>(arg);
            return trigger;
        }
            ...
            Expression<Func<GXTrigger, object?>>? columns)
        {
            ...
            foreach (GXTrigger trigger in triggers)
            {
                ...
                if (trigger.TriggerGroups == null || !trigger.TriggerGroups.Any())
                {
                    ...
                    trigger.TriggerGroups.AddRange(await _triggerGroupRepository.ListAsync(request, null, CancellationToken.None));
                ...
                else
                {
                    ...
                    using (IServiceScope scope = _serviceProvider.CreateScope())
                    {
                        ...
                        triggerGroups = await triggerGroupRepository.GetJoinedTriggerGroups(trigger.Id);
                    }
                    ...
                    updates[trigger] = await GetUsersAsync(trigger.Id);
                }
            }
            foreach (var it in updates)
            {
        ...
        public async Task<bool> RefrestAsync()
        {
            ...
            foreach (var it in updates)
            {
                await _eventsNotifier.TriggerUpdate(it.Value, [it.Key]);
            }
            return list.Any();
        }
    }
