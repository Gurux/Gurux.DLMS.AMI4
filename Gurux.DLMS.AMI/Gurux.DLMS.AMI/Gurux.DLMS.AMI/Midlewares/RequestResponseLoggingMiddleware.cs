...
namespace Gurux.DLMS.AMI.Server.Midlewares
{
    ...
    internal class RequestResponseLoggingMiddleware
    {
            ...
            IPerformanceRepository performanceRepository)
        {
            ...
            if (ServerSettings.GetDefaultAdminUser(host) != null)
            {
            ...
            bool force = false)
        {
            ...
            if (savedValue != null)
            {
                ...
                {
                    await _performanceRepository.AddAsync([savedValue]);
                }
            }
            return value;
        }
            ...
            string text)
        {
            try
            {
                ...
                GXSelectArgs args = GXSelectArgs.Select<GXUser>(s => "*", where => where.Id == id);
                ...
                await enumTypeRepository.GetLogTypeAsync(TargetType.UserAction, action.ToString());
                GXInsertArgs arg = GXInsertArgs.Insert(log);
                await _host.Connection.InsertAsync(arg);
            }
            catch (Exception)
            {
