    ...
    internal sealed class GXWorkflowService : IWorkflowHandler
    {
        ...
        private async void WorkflowJobHandler()
        {
            ...
            using (IServiceScope scope = _serviceProvider.CreateScope())
            {
                ...
                if (cp != null)
                {
                    items = await workflowRepository.ListAsync(req, null, true);
                }
            }
            lock (Workflows)
            {
                ...
                if (item != null)
                {
                    ...
                    try
                    {
                        if (item.ScriptMethods == null || item.ScriptMethods.Count == 0)
                        {
                            ...
                            using IServiceScope scope = _serviceProvider.CreateScope();
                            IWorkflowRepository workflowRepository = scope.ServiceProvider.GetRequiredService<IWorkflowRepository>();
                            GXWorkflow tmp = await workflowRepository.ReadAsync(item.Id, true);
                        ...
                        foreach (GXScriptMethod method in item.ScriptMethods)
                        {
                            ...
                            catch (TargetInvocationException ex)
                            {
                                ...
                                using (IServiceScope scope = _serviceProvider.CreateScope())
                                {
                                    ...
                                    await repository.AddAsync("Job", new GXWorkflowLog[] { error });
                                }
                            }
                            catch (Exception ex)
                            {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        using (IServiceScope scope = _serviceProvider.CreateScope())
                        {
                            ...
                            await systemErrorRepository.AddAsync(TargetType.Workflow, [error]);
                        }
                    }
                }
            }
