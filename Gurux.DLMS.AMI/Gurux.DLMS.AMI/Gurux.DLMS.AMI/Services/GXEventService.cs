...
namespace Gurux.DLMS.AMI.Services
{
    ...
    public class GXEventService : IGXEventsNotifier, IGXEventsListener
    {
        ...
        /// <inheritdoc/>
        public event Action<IEnumerable<GXContent>>? OnContentUpdate;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXContent>>? OnContentDelete;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXContent>>? OnContentClose;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXContentGroup>>? OnContentGroupUpdate;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXContentGroup>>? OnContentGroupDelete;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXContentType>>? OnContentTypeUpdate;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXContentType>>? OnContentTypeDelete;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXContentTypeGroup>>? OnContentTypeGroupUpdate;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXContentTypeGroup>>? OnContentTypeGroupDelete;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXMenu>>? OnMenuUpdate;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXMenu>>? OnMenuDelete;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXMenuGroup>>? OnMenuGroupUpdate;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXMenuGroup>>? OnMenuGroupDelete;
        /// <inheritdoc/>
        ...
        /// <inheritdoc/>
        public event Action<IEnumerable<GXNotification>>? OnNotificationUpdate;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXNotification>>? OnNotificationDelete;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXNotification>>? OnNotificationCalculate;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXNotification>>? OnNotificationClear;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXNotificationGroup>>? OnNotificationGroupUpdate;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXNotificationGroup>>? OnNotificationGroupDelete;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXNotificationLog>>? OnAddNotificationLog;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXNotification?>>? OnClearNotificationLog;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXNotificationLog>>? OnCloseNotificationLog;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXAppearance>>? OnAppearanceUpdate;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXAppearance>?>? OnAppearanceDelete;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXLocalizedResource>>? OnLocalizedResourceUpdate;
        /// <inheritdoc/>
        public event Action<IEnumerable<GXLocalizedResource>>? OnLocalizedResourceDelete;
        ...
        /// <inheritdoc/>
        public async Task ContentClose(IReadOnlyList<string> users, IEnumerable<GXContent> contents)
        {
            OnContentClose?.Invoke(contents);
            await _hubContext.Clients.Users(users).ContentClose(contents);
        }

        /// <inheritdoc/>
        public async Task ContentDelete(IReadOnlyList<string> users, IEnumerable<GXContent> contents)
        {
            OnContentDelete?.Invoke(contents);
            await _hubContext.Clients.Users(users).ContentDelete(contents);
        }

        /// <inheritdoc/>
        public async Task ContentGroupDelete(IReadOnlyList<string> users, IEnumerable<GXContentGroup> groups)
        {
            OnContentGroupDelete?.Invoke(groups);
            await _hubContext.Clients.Users(users).ContentGroupDelete(groups);
        }

        /// <inheritdoc/>
        public async Task ContentGroupUpdate(IReadOnlyList<string> users, IEnumerable<GXContentGroup> groups)
        {
            OnContentGroupUpdate?.Invoke(groups);
            await _hubContext.Clients.Users(users).ContentGroupUpdate(groups);
        }

        /// <inheritdoc/>
        public async Task ContentUpdate(IReadOnlyList<string> users, IEnumerable<GXContent> contents)
        {
            OnContentUpdate?.Invoke(contents);
            await _hubContext.Clients.Users(users).ContentUpdate(contents);
        }

        /// <inheritdoc/>
        public async Task ContentTypeDelete(IReadOnlyList<string> users, IEnumerable<GXContentType> contents)
        {
            OnContentTypeDelete?.Invoke(contents);
            await _hubContext.Clients.Users(users).ContentTypeDelete(contents);
        }

        /// <inheritdoc/>
        public async Task ContentTypeGroupDelete(IReadOnlyList<string> users, IEnumerable<GXContentTypeGroup> groups)
        {
            OnContentTypeGroupDelete?.Invoke(groups);
            await _hubContext.Clients.Users(users).ContentTypeGroupDelete(groups);
        }

        /// <inheritdoc/>
        public async Task ContentTypeGroupUpdate(IReadOnlyList<string> users, IEnumerable<GXContentTypeGroup> groups)
        {
            OnContentTypeGroupUpdate?.Invoke(groups);
            await _hubContext.Clients.Users(users).ContentTypeGroupUpdate(groups);
        }

        /// <inheritdoc/>
        public async Task ContentTypeUpdate(IReadOnlyList<string> users, IEnumerable<GXContentType> contents)
        {
            OnContentTypeUpdate?.Invoke(contents);
            await _hubContext.Clients.Users(users).ContentTypeUpdate(contents);
        }

        /// <inheritdoc/>
        public async Task MenuDelete(IReadOnlyList<string> users, IEnumerable<GXMenu> menus)
        {
            OnMenuDelete?.Invoke(menus);
            await _hubContext.Clients.Users(users).MenuDelete(menus);
        }

        /// <inheritdoc/>
        public async Task MenuGroupDelete(IReadOnlyList<string> users, IEnumerable<GXMenuGroup> groups)
        {
            OnMenuGroupDelete?.Invoke(groups);
            await _hubContext.Clients.Users(users).MenuGroupDelete(groups);
        }

        /// <inheritdoc/>
        public async Task MenuGroupUpdate(IReadOnlyList<string> users, IEnumerable<GXMenuGroup> groups)
        {
            OnMenuGroupUpdate?.Invoke(groups);
            await _hubContext.Clients.Users(users).MenuGroupUpdate(groups);
        }

        /// <inheritdoc/>
        public async Task MenuUpdate(IReadOnlyList<string> users, IEnumerable<GXMenu> menus)
        {
            OnMenuUpdate?.Invoke(menus);
            await _hubContext.Clients.Users(users).MenuUpdate(menus);
        }
        /// <inheritdoc/>
        ...
        /// <inheritdoc/>
        public async Task NotificationClear(IReadOnlyList<string> users, IEnumerable<GXNotification> notifications)
        {
            OnNotificationClear?.Invoke(notifications);
            await _hubContext.Clients.Users(users).NotificationClear(notifications);
        }

        /// <inheritdoc/>
        public async Task NotificationDelete(IReadOnlyList<string> users, IEnumerable<GXNotification> notifications)
        {
            OnNotificationDelete?.Invoke(notifications);
            await _hubContext.Clients.Users(users).NotificationDelete(notifications);
        }

        /// <inheritdoc/>
        public async Task NotificationGroupDelete(IReadOnlyList<string> users, IEnumerable<GXNotificationGroup> groups)
        {
            OnNotificationGroupDelete?.Invoke(groups);
            await _hubContext.Clients.Users(users).NotificationGroupDelete(groups);
        }

        /// <inheritdoc/>
        public async Task NotificationGroupUpdate(IReadOnlyList<string> users, IEnumerable<GXNotificationGroup> groups)
        {
            OnNotificationGroupUpdate?.Invoke(groups);
            await _hubContext.Clients.Users(users).NotificationGroupUpdate(groups);
        }

        /// <inheritdoc/>
        public async Task NotificationUpdate(IReadOnlyList<string> users, IEnumerable<GXNotification> notifications)
        {
            OnNotificationUpdate?.Invoke(notifications);
            await _hubContext.Clients.Users(users).NotificationUpdate(notifications);
        }

        /// <inheritdoc/>
        public async Task AddNotificationLogs(IReadOnlyList<string> users, IEnumerable<GXNotificationLog> logs)
        {
            OnAddNotificationLog?.Invoke(logs);
            await _hubContext.Clients.Users(users).AddNotificationLogs(logs);
        }
        /// <inheritdoc/>
        public async Task ClearNotificationLogs(IReadOnlyList<string> users, IEnumerable<GXNotification>? logs)
        {
            OnClearNotificationLog?.Invoke(logs);
            await _hubContext.Clients.Users(users).ClearNotificationLogs(logs);
        }

        /// <inheritdoc/>
        public async Task AppearanceDelete(IReadOnlyList<string>? users, IEnumerable<GXAppearance>? appearances)
        {
            OnAppearanceDelete?.Invoke(appearances);
            await _hubContext.Clients.All.AppearanceDelete(appearances);
        }

        /// <inheritdoc/>
        public async Task AppearanceUpdate(IReadOnlyList<string>? users, IEnumerable<GXAppearance> appearances)
        {
            OnAppearanceUpdate?.Invoke(appearances);
            await _hubContext.Clients.All.AppearanceUpdate(appearances);
        }

        /// <inheritdoc/>
        public async Task LocalizedResourceDelete(IReadOnlyList<string>? users, IEnumerable<GXLocalizedResource> resources)
        {
            OnLocalizedResourceDelete?.Invoke(resources);
            await _hubContext.Clients.All.LocalizedResourceDelete(resources);
        }

        /// <inheritdoc/>
        public async Task LocalizedResourceUpdate(IReadOnlyList<string>? users, IEnumerable<GXLocalizedResource> resources)
        {
            OnLocalizedResourceUpdate?.Invoke(resources);
            await _hubContext.Clients.All.LocalizedResourceUpdate(resources);
        }
    }
}
