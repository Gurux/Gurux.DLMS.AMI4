...
namespace Gurux.DLMS.AMI.Server.Services
{
    ...
    internal sealed class GXSubtotalService : ISubtotal, IHostedService
    {
        ...
        /// <inheritdoc/>
        public void Calculate(IEnumerable<Guid>? subtotals)
        {
            lock (_subtotals)
            {
                if (subtotals == null)
        ...
        /// <inheritdoc/>
        public void Cancel(IEnumerable<Guid>? subtotals)
        {
            lock (_subtotals)
            {
                if (subtotals == null)
        ...
        public async Task UpdateAsync()
        {
            ...
            if (User != null)
            {
                ...
                var subtotals = await repository.ListAsync(req, null, CancellationToken.None);
                foreach (var subtotal in subtotals)
                {
                    if (subtotal.Last != null)
                    {
                        ...
                        if (tmp <= now)
                        {
                            Calculate(new Guid[] { subtotal.Id });
            ...
            List<GXSubtotalValue> list)
        {
            if (list.Any())
            {
                ...
                GXSubtotalValue[] ret = await subtotalValueRepository.ListAsync(req2, null, CancellationToken.None);
                if (ret.Any())
                {
                ...
                {
                    //Add subtotal items.
                    await subtotalValueRepository.AddAsync(items);
                ...
                subtotal.Agents.Any())
            {
                ...
                foreach (var id in agentIds)
                {
                    ...
                    else if (subtotal.Type == (int)SubtotalType.Connect)
                    {
                        using (IServiceScope scope = _serviceProvider.CreateScope())
                        {
                            var repository = scope.ServiceProvider.GetRequiredService<IEnumTypeRepository>();
                            int type = await repository.GetLogTypeAsync(TargetType.SystemLog, "Connected");
                            arg.Where.And<GXAgentLog>(w => w.Type == type);
                        }
                    }
                    //Get disconnections.
                ...
                try
                {
                    ...
                    if (s != null)
                    {
                        using (IServiceScope scope = _serviceProvider.CreateScope())
                        {
                            ...
                            var it = await subtotalRepository.ReadAsync(s.Value.subtotal);
                            if (it.TraceLevel > TraceLevel.Warning)
                            {
                                ...
                                await repository.AddAsync("Count", [log]);
                            }
                            ...
                            if (updated != null)
                            {
                                ...
                                await subtotalRepository.UpdateAsync([it], c => new { c.Last, c.Status });
                            }
                            else
                            {
                                //Update status.
                                it.Status = SubtotalStatus.Idle;
                                await subtotalRepository.UpdateAsync([it], c => c.Status);
                            }
                            if (it.TraceLevel > TraceLevel.Warning)
                            {
                                GXSubtotalLog log = new GXSubtotalLog(TraceLevel.Info)
                                ...
                                await repository.AddAsync("Count", new GXSubtotalLog[] { log });
                            }
                        }
                    }
                }
                catch (Exception ex)
                {
                    try
                    {
                        if (s != null)
                        {
                            ...
                            {
                                ISubtotalRepository subtotalRepository = scope.ServiceProvider.GetRequiredService<ISubtotalRepository>();
                                await subtotalRepository.UpdateAsync([it], c => new { c.Status });
                                ISubtotalLogRepository repository = scope.ServiceProvider.GetRequiredService<ISubtotalLogRepository>();
                                await repository.AddAsync("Count", [log]);
                            }
                        }
