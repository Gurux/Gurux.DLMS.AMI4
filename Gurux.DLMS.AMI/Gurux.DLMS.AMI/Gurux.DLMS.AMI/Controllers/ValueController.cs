...
namespace Gurux.DLMS.AMI.Server.Repository
{
    ...
    public class ValueController : ControllerBase
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            await _valueRepository.ListAsync(request, ret, cancellationToken);
            return ret;
        }
        ...
        public async Task<ActionResult<AddValueResponse>> Post(AddValue request)
        {
            ...
            await _valueRepository.AddAsync(request.Values);
            return ret;
        }
        ...
        public async Task<ActionResult<ClearValueResponse>> Post(ClearValue request)
        {
            ...
            {
                await _valueRepository.ClearDeviceAsync(request.Devices);
            }
            if (request.Objects != null && request.Objects.Any())
            {
                await _valueRepository.ClearObjectAsync(request.Objects);
            }
            if (request.Attributes != null && request.Attributes.Any())
            {
                await _valueRepository.ClearAttributeAsync(request.Attributes);
            }
