...
namespace Gurux.DLMS.AMI.Server.Controllers
{
    ...
    public class ConfigurationController : ControllerBase
    {
        ...
        public async Task<ActionResult<GetConfigurationResponse>> Get(Guid id)
        {
            ...
            {
                Item = await _configurationRepository.ReadAsync(id, null)
            };
        }
            ...
            CancellationToken cancellationToken)
        {
            ...
            await _configurationRepository.ListAsync(request, ret, cancellationToken);
            return ret;
        }
        ...
        public async Task<ActionResult<ConfigurationUpdateResponse>> Post(ConfigurationUpdate request)
        {
            ...
            await _configurationRepository.UpdateAsync(request.Configurations, true);
        ...
        /// <returns></returns>
        [HttpPost("Prune")]
        [Authorize(Policy = GXConfigurationPolicies.Prune)]
        public async Task<ActionResult<ConfigurationRunPruneResponse>> Post(ConfigurationRunPrune request)
        {
            IGXPruneService pruneService = _serviceProvider.GetRequiredService<IGXPruneService>();
            await pruneService.RunAsync();
            return new ConfigurationRunPruneResponse();
        }
        ...
        public async Task<ActionResult<LoadedAssembliesResponse>> Post(LoadedAssemblies request)
        {
            ...
            foreach (Assembly assembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                try
                {
                    ...
                    {
                        //If this is a script.
                        GXScript script = await _scriptRepository.ReadAsync(id);
